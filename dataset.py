...
class CatSegmentationDataset(Dataset):
    ...
    def __init__(
            self,
            images_dir = r'C:/Users/WenBi/Desktop/Project/24723-Pytorch/seg/cats_output/',
            image_size=32,
    ):
        print("Reading images...")
        # 原图所在的位置
        image_root_path = images_dir + '/JPEGImages'
        # Mask所在的位置
        mask_root_path = images_dir + '/SegmentationClassPNG'
        # 将图片与Mask读入后，分别存在image_slices与mask_slices中
        self.image_slices = []
        self.mask_slices = []
        for im_name in os.listdir(image_root_path):
            mask_name = im_name.split('.j')[0] + '.png'
